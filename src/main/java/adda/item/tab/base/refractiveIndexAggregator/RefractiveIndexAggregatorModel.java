...
//    protected ShapeSelectorModel shapeModel;
    ...
    private void crossValidation() {
        ...
        if (formulationModel != null && formulationModel.getPolarization() == PolarizationEnum.cldr) {
        ...
        if (formulationModel != null && formulationModel.getInteraction() == InteractionEnum.so) {
            isEnableAnisotropy = false;
            error += StringHelper.toDisplayString("<html>Anisotropy refractive index <br>does`t compatible with SO interaction</html>");
        }
        if (dipoleShapeModel != null && dipoleShapeModel.getEnumValue() == DipoleShapeEnum.Rect) {
            isEnableAnisotropy = false;
